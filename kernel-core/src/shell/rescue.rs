//! Rescue/diagnostic shell commands (CONFIG_RESCUE_REPAIR): filesystem
//! listings, mount aliases, disk health and the boot-repair entry point.
//! Read-only except `grub-fix`, whose repair/install paths enable repair
//! mode only after an explicit YES confirmation. The module (and every
//! command registration) is absent from the minimal kernel; the core help +
//! bootinfo live in shell/cmds.rs.
