//! Graphics demo task (M13-2, i686): a bounded kernel task that drives the
//! shared framebuffer demo on the VBE console, reports its per-frame damage
//! accounting on serial through the no-mirror sink (so it never touches the
//! screen), then exits and is reaped. The markers are built into a stack
//! buffer by hand like the M3 demos (no `core::fmt` on the 16 KiB task stack).
//! On a headless boot the console access returns `None` and the task just
//! prints its markers (serial parity holds).
...
pub fn run() -> ! {
    ...
    for frame in 0..FRAMES {
        if let Some(stats) = crate::fb::gfx_demo_frame(frame) {
