...
**ICMP echo client** for the Live kernel's userspace. Catalog skeleton (C5):
...
- shell command: `ping <host> [count]` in `kernel/src/shell/cmds_net.rs`,
  registered only when `CONFIG_TOOLS=y`;
- client: `ping_once` in `kernel-net` (`CONFIG_NET=y`), sequenced by the
  request slot `kernel-net/src/c/rump_tools.c`;
...
into `src/` (plus replayable `patches/`), and `CONFIG_APP_PING=y` retires the
in-kernel command.
