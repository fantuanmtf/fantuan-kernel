//! Graphics demo task (M13-2/M13-3): a bounded kernel task that drives the
//! shared framebuffer demo on the GOP console (moving box + counter + a
//! pointer-driven cursor sprite), reports its per-frame damage accounting on
//! serial through the no-mirror sink (so it never touches the screen), and
//! consumes the input event ring: pointer events move the cursor, and a `q`/
//! `Esc` key press stops the animation early. The markers are built into a
//! stack buffer by hand like the M3 demos, since `core::fmt` is avoided on the
//! 16 KiB task stack.
...
pub fn run() -> ! {
    ...
    let (w, h) = crate::console::gfx_demo_dims().unwrap_or((1280, 800));
    ...
    for frame in 0..FRAMES {
        ...
        let stats = crate::console::gfx_demo_frame(frame);
        let cur = crate::console::gfx_demo_cursor(old_cx, old_cy, cx, cy);
        crate::console::gfx_demo_present();
        match stats {
            Some(s) => {
