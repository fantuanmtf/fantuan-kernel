...
- [x] M13-2 double buffer + present + kernel demo app (V-b)
- [x] M13-3 input event ring + PS/2 mouse (V-c)
- [x] M13-4 dumb buffers + ADDFB/SETCRTC/PAGE_FLIP + events (V-d)
...
| Date | Check | Result |
|---|---|---|
| 2026-09 | P4: the console lands in the **login shell** (bash), not in the built-in shell. `kernel_core::shell` gained a login-shell hook (`set_login_shell`, the same shape as `set_bin_lookup`/`set_auth_apply`) and `kernel/src/shell/sh.rs::login()` runs bash — else dash — as a foreground user task on `/dev/console`, with `USER=root`, `HOME=/root`, `HOSTNAME=Fantuan-MTF`, `PATH=/bin:/usr/bin` and a `PS1` that renders `root@Fantuan-MTF:/# `; `exit` returns to the built-in shell, which announces itself (`shell: login shell exited — built-in rescue shell (...)`) and still answers `sh`, so nothing becomes unreachable. Arches without an embedded shell (riscv64, aarch64, i686), `CONFIG_BASH=n` builds and stale-artifact trees never install the hook and keep landing in the built-in shell, unchanged. Ordering that the first cut got wrong and the gates caught: the ESP autorun is queued by `load_script` and fed by `read_line` *inside* the shell loop, so handing the console over before draining the queue silently skipped every `--shell-repair` fixture — the loop body is now a `prompt_and_run` helper that the autorun drain and the interactive loop share, which preserves the transcript (`root@Fantuan-MTF> grub-fix repair`) and runs the script to completion first. What deliberately does **not** change is where the tools live: `lsos`/`mount`/`diskhealth`/`grub-fix`/`clone`/`gpu` are kernel built-ins, so the autorun still runs in the built-in shell and they are not yet on `PATH` — that is M13-6's repair broker, and until it lands bash cannot run them (recorded in DESIGN §10). Smokes adapted to the landing: `smoke-bash` asserts the login shell, drives bash in place and then exercises `exit` → built-in → `sh` re-entry; `smoke-dash` starts dash from bash and keeps the built-in reap path in the post-`exit` leg; `smoke-config`'s `boot_help` types `exit` before `help`; `smoke-net`/`-tls` drop to the built-in shell before driving the kernel tools; `kbd_test` expects bash's `command not found` for the injected line; `smoke.sh` asserts the login-shell marker instead of the built-in idle notice. Every smoke now builds once and boots with `run.sh --no-build` (the default build is no longer free — it builds the shells), so a phase timeout measures boot time; `smoke.sh`'s broken-ESP phase is what caught that. Verified: `tools/smoke.sh` 13/13 phases PASS, `tools/smoke-config.sh` PASS (all phases), `tools/smoke-posix.sh` PASS, `tools/smoke-bash.sh` PASS, `tools/smoke-dash.sh` PASS, `tools/smoke-net.sh` PASS, `tools/kbd_test.sh` PASS, plus the boot transcript by hand (prompt, `exit`, `sh` re-entry). | PASS |
| 2026-09 | gates: the serial log is line-atomic and NUL-free, so the marker-based checks can actually read it. `smoke.sh`'s Secure Boot phase was red for two reasons unrelated to Secure Boot, and both broke *matching* rather than the feature: (1) the virtualization probe printed the raw 12-byte CPUID hypervisor vendor (`KVMKVMKVM\0\0\0`) and three NUL bytes are enough for `grep` to classify the whole log as binary - after which a plain `grep -q MARKER log` reports no match even when the line is right there; (2) the graphics demo and the shell/boot-repair path share COM1 while both deliberately bypass the serial `LOCK` (a `sys_write` in an interrupt gate spins on it, so holding it across a preemption would deadlock), so their output interleaved byte-wise - the repair line came out as `se` + `cureboot: SetupMode ACTIVE`. Fixed by stopping the vendor print at the first NUL (the TCG comparison still uses the raw field) and by running each `log_bytes`/`log_bytes_raw` call with interrupts disabled: on this single-CPU target nothing else can run while the flag is clear, and `putc`'s wait is bounded by the UART rather than by another task. The coupling is worth naming: a log the gates cannot read is not evidence. Verified: all seven Secure Boot markers present in a fresh-store `--smm --shell-repair` run with a 0-NUL log (was 3), `tools/smoke-kms.sh` PASS, `tools/smoke.sh` 13/13 phases PASS. | PASS |
| 2026-09 | governance: the two dormant branches are rebuilt and the GPL blob is purged from history. `fantuan-apps` and `package` were C2 skeletons 23 commits behind `main` (400 files of drift) and are now rebuilt from the current `main` with `tools/mkbranches.sh`: `fantuan-apps` carries the catalog plus the bash source bundle (`apps/bash/src/{bash-5.3.tar.gz,.sig}`, bytes equal to the pin `0d5cd869…`) and a branch-local `apps.lock` whose bash entry is recomputed for the branch tree, `package` carries the appctl client, both carry a CI that runs `verify --apps-layer` (without the flag it would be red the moment a GPL app is in the tree). The skeleton ignores untracked files in the dirtiness check now, since branch trees come from `read-tree`, not the working tree. History: `git filter-branch --index-filter` (git's own tooling, no external script) removed `apps/bash/src/bash-5.3.tar.gz{,.sig}` from every commit; the old objects stayed reachable through a **stale linked worktree** (`/tmp/opencode/head`, detached at P2) which `git worktree prune` removed, after which reflogs were expired and `gc --prune=now` ran: `.git` went 19 MB -> 8.1 MB and `git rev-list --objects main` no longer reaches the blob at all. Published with `--force-with-lease`: GitHub and Codeberg took `main` (`5f8942b -> 19981b6`), GitHub took both branches; **GitLab took the branches but rejected `main`** because it protects that branch against force pushes — recorded in `docs/REPO_POLICY.md` §1 as an owner action, since a mirror one rewrite behind is worse than a red gate. `tools/smoke-gpl.sh` now passes end to end including `[a4]`, which asserts the published `fantuan-apps` bytes against the pin. | PASS |
| 2026-09 | licensing: main carries no GPL source, and the bash bundle moves to the fantuan-apps branch. `apps/bash/src/bash-5.3.tar.gz` + `.sig` left the `main` tree (the metadata tree stays: manifest, README, `COPYING`, `patches/`, `port/`, `src/SOURCE`, `src/SHA256SUMS`); a history rewrite purges the blobs from the whole history as well, so the rule holds for every commit, not just the tip. The manifest gained `source_provision` (upstream URL + sha256 + mirror + patch) and `apps.lock` was recomputed with the new `appctl relock` verb — which also fixed the **pre-existing stale entry** (`afc73162…` against the real tree `10d85de0…`) that made `appctl verify --apps-layer` fail, i.e. the licensing gate was red before this batch. `tools/smoke-gpl.sh` was rewritten to be both honest and stricter: `[a1]` the tip tree is metadata-only (no archive under `apps/bash/`, `src/` holds only SOURCE + SHA256SUMS, < 256 KiB tracked) and a locally fetched tarball must be gitignored; `[a2]` the three source pins agree without the tarball; `[a3]` the tarball is fully checked when present and an explicit `SKIP` is printed when it is not; `[a4]` the `fantuan-apps` mirror carries the pinned bytes and the same port patch; `[b]` unchanged plus a lock-vs-tree assertion; `[e]` proves no bash *code* is linked (anchored `nm`, app-path `strings`) **and** positively asserts that the image embeds the recorded artifact hash, instead of claiming the image is bash-free. `app_vendor` no longer drops `COPYING`/`port` when vendoring (that is why a re-vendor could not reproduce the lock hash). `docs/REPO_POLICY.md` records the remote roles (GitHub primary, GitLab full mirror, Codeberg pure-kernel) and the licence layers; `THIRD_PARTY.md`, `docs/APPS.md`, `apps/README.md`, `apps/bash/README.md`, `docs/HANDOVER.md` and `docs/M14_LINUXUSERS.md` (file-based bash delivery + `/usr/src/bash/` as the M14 task) were updated to match the tree. Verify `tools/smoke-gpl.sh` PASS, `tools/smoke-apps.sh` PASS. | PASS |
| 2026-09 | default shell: bash is built and embedded by `tools/build.sh` itself. The default x86_64 path now runs `build-libc.sh` + `build-bash.sh` + `build-dash.sh` when `CONFIG_BASH=y` and exports `FANTUAN_REQUIRE_BASH`, so a missing artifact is a build error instead of a silent dash fallback; `kernel/build.rs` honours `CONFIG_BASH=n` (never embeds a stale blob) and bakes the artifact hash in as `SHELL_IMAGE_SHA256` — deliberately not named after the shell, because the licensing gate greps the ELF for shell symbols — which the kernel prints at boot as `shell: image sha256 …`. Sources come from the new `tools/fetch-bash-src.sh` (env override → `build/cache/` → a vendored tree → the pinned upstream URL with sha256 + GPG → `--from-branch`), and `tools/build-bash.sh` now verifies that every manifest-listed patch was applied before compiling and writes `kernel/bash_program.sha256`. Both shell builders gained an input stamp, so a repeat `tools/build.sh` skips configure+make: the default build is 16 s instead of 2m48s, which is also what keeps `smoke-config`'s 90 s boot budget honest. Gates: `smoke-bash` phase `[0/5]` deletes the artifacts and proves that `tools/build.sh` **alone** builds and embeds bash and that the marker is inside the ELF; `smoke-config` asserts the minimal profile embeds the marker, adds `[phase no-shell]` (with `BASH=n` the stage is skipped and the ELF carries no shell bytes) and counts `kernel/bash_program.bin` in the 300 MiB budget. Also fixed a pipefail footgun the M13-3 string checks had: `strings | grep -q` reads as *not found* when it matches, because the early `grep` exit SIGPIPEs `strings` — the new `elf_has_substring` helper avoids the pipe. Docs: `POSIX_PLAN.md` (default-shell decision + what P2 meant), `USAGE.md`, `CONFIG_PLAN.md`, `BUILD.md` (§1 toolchain incl. bison, §3.1/§4, §7 troubleshooting), `config/Kconfig`, `README.md`. Verify `tools/smoke-bash.sh` PASS (phase 0 + all transcripts), `tools/smoke-config.sh` PASS. | PASS |
| 2026-09 | M13-4 V-d dumb buffers + ADDFB/SETCRTC/PAGE_FLIP + gfx events: `kernel-core::graphics::kms` (`CONFIG_GRAPHICS`, 298 lines): a bounded dumb-buffer pool (`MAX_DUMB = 8`, each buffer a contiguous run from the frame allocator reached through the `phys_to_virt` alias, slot reserved before allocation so a full pool cannot leak frames) with `dumb_create`/`dumb_destroy`/`dumb_info`/`dumb_used`; a framebuffer registry (`MAX_FB = 16`) with `addfb` (validates `pitch >= w*bpp` and `pitch*h` against the allocation, and format equality)/`rmfb`/`fb_info`; the CRTC seam `set_scanout`/`scanout`/`crtc_fb` with `setcrtc`/`page_flip` validating against the CRTC mode and reporting `KmsError::{NoScanout,BadFb,GeometryMismatch}`; three `static mut` tables behind one `AtomicBool` + `arch::IrqLock`, no allocation, assert-free. `kernel-core::graphics::event`: a frozen `#[repr(C)]` 32-byte `GfxEvent` (ts/kind/code/fb_id/seq/_reserved, `kind::{FLIP_COMPLETE,ERROR}`, `code::{NONE,NO_SCANOUT,BAD_FB,GEOMETRY_MISMATCH}`) in a bounded 64-entry ring with drop-oldest + a monotonic `dropped()` counter; `selftest` guards the ABI size, FIFO order, an ERROR round-trip and the drop policy at boot (`gfx: event ring self-test ok`). The console publishes its GOP surface as the CRTC scanout at boot and exposes `gfx_kms_ready()`/`gfx_restore()`; `kernel/src/kms_demo.rs` takes the screen from `gfx_demo` only when the console is double-buffered, allocates two dumb buffers at the mode geometry, registers both, `setcrtc`s the first, runs 8 render/`page_flip` cycles each confirmed by a popped `FLIP_COMPLETE`, proves the geometry-mismatch negative (`gfx: setcrtc mismatch ok`), frees everything and asserts `dumb_used()==0` plus the allocator's usable MiB back at the pre-demo baseline (`gfx: cleanup ok`), then restores the console surface and unfreezes the mirror (all markers over the no-mirror sink). `tools/smoke-kms.sh` (rescue profile, `--vga std`, serial relayed over a unix socket so the markers are not buffered) asserts the whole sequence, exactly eight `page_flip` lines and no panic, and requires two screendumps at the flip markers to differ with their bbox inside the reported framebuffer geometry (3 attempts, absorbing the known QEMU monitor timing flake). Verified: `tools/smoke-kms.sh` PASS (re-run after the `unwrap`/dead-state cleanup in `kms_demo.rs`), `tools/smoke-config.sh` PASS (the minimal profile carries none of the six `gfx:` markers), `cargo check` x86_64 + kernel-core riscv64 zero warnings. Deferred: the §4 ioctl numbers/GETRESOURCES/GETCONNECTOR/EDID (M13-5) and the userland fd boundary (M14); `kms.rs` is at 298/300 lines, so split before extending it | PASS |
| 2026-09 | M13-3 V-c input ring + PS/2 mouse (working tree): `kernel-core::input_ring` (a frozen 32-byte `InputEvent` layout — ts/kind/flags/set/scancode/ascii/buttons/dx/dy — the M13-3 contract, size-guarded by the boot self-test `input: ring self-test ok`; one lock-protected ring of 64 events, IRQ-safe lock, drop-oldest + monotonic `dropped()` counter, no allocation) and `kernel-core::mouse` (the pure 3-byte packet decoder with sign/overflow bits), both `CONFIG_GRAPHICS`-gated so minimal/net link neither. x86_64 `kernel/src/mouse.rs` and i686 `kernel-i686/src/mouse.rs` bring up the 8042 aux port (enable aux + IRQ12, config byte preserved from the keyboard init, reset with bounded ACK/self-test/ID reads, defaults, 3-byte reporting) and feed `push_pointer`; the C i8042 keyboard now returns a packed key event on IRQ1, which the dispatch feeds into the ring while the ASCII shell path is unchanged. The demo consumes pointer events to move a magenta cursor sprite (drawn in the bottom band so the damage bbox stays tight) and a `q`/`Esc` key press to stop early (`input: demo consumed dx=.. dy=.. buttons=..`, `input: demo cursor x=.. y=..`, `input: demo key stop scancode=.. ascii=..`). `tools/smoke-input.sh` PASS (QEMU monitor `mouse_move 12 5` -> `dx=12 dy=-5` (QEMU inverts PS/2 Y), cursor `x=28 y=699`, `sendkey q` -> `scancode=16 ascii=113` early stop, screendump diff contained in the reported damage); `smoke-graphics.sh` PASS, `smoke-bios.sh` 2/2, `smoke-config.sh` PASS (minimal free of `ring self-test`/`PS/2 aux`), `smoke-gpu.sh` PASS; x86_64 minimal/rescue/net/tls, i686, riscv64, aarch64 builds zero warnings. Deferred: wheel/middle button consumers, IRQ sharing edge cases, i686 runtime input (built and booted, not driven). Next: V-d dumb buffers + ADDFB/SETCRTC/PAGE_FLIP + events | PASS |
| 2026-09 | M13-2 V-b double buffer + demo (working tree): `kernel-core::graphics` gained `BufferedPresent` (copies only the damaged rects from an off-screen `FbInfo` to the scanout then clears; `blit`'s source-pointer convention documented), `Damage::{bbox,area}` and a shared `demo_frame` (moving box + 8-bit counter in a bottom band) plus a `BufferedPresent` round-trip in `selftest`. Both consoles start single-buffered and `upgrade_buffered` swaps them to a frame-allocator back buffer after the allocator is up (bounded to the mode's exact size, falls back to `DirectPresent` on failure, so the i686 `510 MiB usable` report is unchanged). A bounded demo task (`kernel/src/gfx_demo.rs`, `kernel-i686/src/gfx_demo.rs`) runs 40 frames on the GOP/VBE, reports each frame's damage over a **no-mirror** serial sink (`graphics: demo frame=N dmg=X,Y,W,H`), then prints `start/stop` + `demo damage total=` + `idle damage empty ok` and exits; x86_64 kmain waits for the userland/proc-test reaps and freezes the GOP mirror while the demo runs (the demo unfreezes), so the box is the only thing on screen. `tools/smoke-graphics.sh` screendumps at known frames (serial relayed unbuffered over a unix socket), asserts the images differ and that the differing pixels lie inside the union of the sampled frames' damage rects, retries the screendump+containment on the (rare) QEMU monitor timing flake, and keeps the i686 serial parity check. `tools/smoke-graphics.sh` PASS ×5 consecutive, `smoke-bios.sh` 2/2, `smoke-config.sh` PASS (minimal graphics-free), `smoke-gpu.sh` PASS; x86_64 minimal/net/tls/rescue, i686, riscv64, aarch64 builds zero warnings | PASS |
...
**M13-5 (V-e): EDID sourcing + the synthetic fallback + connector
properties.** V-d just landed the KMS-like interior (dumb buffers,
ADDFB/SETCRTC/PAGE_FLIP, the gfx event ring, the demo proving it). The next
step is the `§4` surface a userspace modesetting driver consumes: the
`DRM_IOCTL_*` numbers and structs, `GETRESOURCES`/`GETCONNECTOR` (with EDID
from GOP/VBE and the built-in 1024x768 fallback blob clearly marked as
synthetic), `GETCRTC`, `CREATEDUMB`/`MAP_DUMB` and the flip-completion event
readable through a `read()`-shaped seam. Watch the two known constraints:
`kernel-core/src/graphics/kms.rs` is at 298/300 lines (split it before
extending) and `kernel/src/main.rs` is at 407 lines, already over the
project's 300-line rule (an independent cleanup, not part of M13-5).
M13-6 (the repair broker + IPC contract) and M13-7 (freezing
`GRAPHICS_API.md`/`REPAIR_IPC.md`) follow and close v0.0.5.
