...
if grep -q "fantuan v0.0.4 (riscv64)" "$LOG" \
  ...
  grep -aE "fantuan v0.0.4 \(riscv64|mm: frame self-test|trap: |timer: |tick: |sched: |task [12] |user: |userland: |cpu: |^exc |blk: |vfs: |ext4: |probe: |bootrepair: |shell|SMART|Hello from" "$LOG" | head -40 || true
else
